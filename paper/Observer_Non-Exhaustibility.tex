...
\textbf{Trust boundary:} Route classification bundles theorems proved in distinct Lean corpora (representational incompleteness, reflective fold obstruction, awareness arc); synthesis language coordinates them but does not collapse their independent hypotheses.

\NovaZenodoInAbstract
\end{abstract}

\noindent\textbf{Keywords:} observer non-exhaustibility, representational incompleteness,
reflective fold obstruction, awareness arc, Lean~4

\noindent\textbf{MSC 2020:} 03F40, 03B70, 68T27

\noindent\textbf{Code/Data availability:}\newline
\begingroup\raggedright
\url{https://github.com/novaspivack/observer-non-exhaustibility-lean}\linebreak
(\path{observer-non-exhaustability-lean} development; \path{lake build}; repository \path{README} and \path{lakefile.lean}.)
\par\endgroup

\section{Premises, disagreement coordinates, and trust boundary}\label{sec:premises-one-flagship}
\begin{keybox}[title=Where disagreement can (and cannot) live]
\textbf{Formal spine.}
Each blocked route cites a concrete engine (\RI{}, \RFO{}, semantic-self-description spine, etc.); the packaging layer only discharges route bookkeeping and conjunction where explicitly proved.

\medskip
\textbf{Premise coordinates (for external readers).}
\begin{itemize}[leftmargin=2em]
\item \textbf{Engine vs.\ packaging.} ``Packaging theorems'' may be thin wrappers; refuting them is not the same as refuting the underlying engine manuscripts.
  \emph{Logical effect:} Dispute the summit by engaging the cited \texttt{.lean} modules, not headline sociology alone.
\item \textbf{Positive survivor architecture.} The awareness-grounded arc imports its own formal prerequisites from the reflexive-closure program; it is not a free-floating empirical prediction.
  \emph{Logical effect:} Opting out of that program's typing conventions leaves the negative routes intact but unsupported positive claims inert.
\end{itemize}
\end{keybox}
...
questions in philosophy and one of the newest in formal science.  It runs
through classical and modern reflection on self-knowledge, formal
self-reference, phenomenology, computation, and disputes over
consciousness, AI, and observers in physics.  Three lineages matter most
here: a \emph{classical and modern subjectivity} thread (Plato, Aristotle,
Augustine, Descartes, Kant, Hegel); a \emph{formal diagonal and reflexivity}
thread (G\"odel, Turing, Lawvere, Yanofsky, Hofstadter); and a
\emph{phenomenology of manifestation} thread (Husserl, Merleau-Ponty).
...
\begin{keybox}[title={The structural gap this paper fills}]
Formal diagonal work often stops at negative obstruction; phenomenology at
descriptive insight; computation at implementation bounds.  The missing
piece is a \textbf{route classification}: which internal strategies could
in principle exhaust an observer, which are blocked (and how), and what
positive architecture remains once they are.
\end{keybox}
...
These figures are cited not because they anticipated the present synthesis
in full, but because they supply \emph{neighboring problems and tools} now
composed in the theorem stack: reflexive self-knowledge and its limits
(classical thread); diagonal and decision-theoretic structure (formal
thread); non-objectifiable presence and manifestation (phenomenological
thread).
...
limits, and artifact structure.  \Cref{app:formalization} tabulates the
\Lean{} packaging declarations aligned with the repository inventory.
...
\begin{keybox}[title={Three layers readers must not conflate}]
\begin{itemize}[leftmargin=1.3em,itemsep=0.45em,parsep=0pt,topsep=2pt]
\item \textbf{(1)~Discharged engines:} base theorems in external \Lean{}
      libraries (\RI, \RFO, \SEM, awareness arc).
\item \textbf{(2)~\ONE{} packaging:} route type, bridge wrappers, summit conjunction,
      four-way exhaustiveness of the route tag, and route-partition residual
      in this repository (\Cref{app:formalization}).
\item \textbf{(3)~Synthesis conclusions:} the prose classification that composes those surfaces;
      the claim that \emph{every} informal theory falls under the partition is
      synthesis-level, not a single packaged lemma.
\end{itemize}
\end{keybox}
...
\item \textbf{\ONE{} packaging theorems.}
      ...
      \item Formal route-tag \textbf{exhaustiveness}
            (\leanref{observer\_exhaustion\_route\_exhaustive\_four}):
            every value of \leanref{ObserverExhaustionRoute} is
            Route~A, B, C, or~D.
      \item Route-partition residual
            (\leanref{route\_partition\_residual\_of\_not\_ABC},
             \leanref{observer\_route\_classification\_residual}):
            within that vocabulary, any tag that is not A,
...
\medskip
\noindent
For declaration-level alignment with the outer repository, see
Appendix~\ref{app:formalization} (mirrors \texttt{MANIFEST.md} and
\texttt{NOTES/FORMALIZATION\_MAP.md}).
\end{remark}
...
\begin{keybox}[title={The guiding question}]
What would full internal exhaustion require, which principal internal
strategies might deliver it, and---once the reductive lanes fail---what
admissible observer architecture is left?
\end{keybox}
...
This is a \textbf{synthesis-level working definition} for purposes of route
classification in this paper, not a claim to a final metaphysical essence
of observerhood.
...
This is a synthesis summit paper: its contribution is integrating
already-discharged theorem engines into a final classification, not
proving new base diagonal, fold, or foundational awareness theorems
(theorem-status layers: \cref{sec:status}).
...
\begin{keybox}[title={Four pillars of the synthesis}]
Representational incompleteness (parametric self-model / diagonal);
reflective fold obstruction (closure vs architectural fold); internal
completion plus semantic self-description (certification and final
theory); awareness arc (non-collapsing, minimally ternary residual).
Each pillar is a discharged engine or program slice composed here into
one classification story.
\end{keybox}

The \ONE{} repository is intentionally thin: a synthesis-and-classification
layer whose \Lean{} packaging certifies route vocabulary and summit
conjunction without duplicating engine proofs (\cref{sec:artifact}).

\begin{remark}
This paper is a synthesis summit, not a new obstruction engine; the
engine vs.\ packaging vs.\ synthesis distinction is fixed once in
\cref{sec:status}.
\end{remark}
...
\begin{keybox}[title={Why Routes A--C exhaust the reductive attack}]
They span the three natural internal ``handles'': how the observer is
\emph{modeled}, what states are \emph{reachable} by internal steps, and
what it can \emph{certify} about itself.  A strategy outside these handles
is not an internal reductive exhaustion attempt in the sense used here; it
already lives in the residual lane.
\end{keybox}
...
Route~C is the \emph{internal-totalization lane}: whether by certification,
self-theory, or self-description, the observer is supposed to achieve
\emph{internal semantic finality}.  The \ICA{} bridge and \SEM{} line block
that ambition under complementary hypotheses; what follows spells this out.

Concretely, one hopes to exhaust observerhood by internal
...
\begin{keybox}[title={Negation alone does not pick out the survivor}]
Blocking Routes A--C shows what observerhood is \emph{not} in terms of
those reductive templates.  A full synthesis must still say what
observerhood \emph{is} in admissible positive form.
\end{keybox}
...
The burden is therefore not merely to reject Routes A--C, but to show that
observerhood admits a positive architecture not already captured by those
reductive forms.  The remainder of the paper discharges that burden.
...
\begin{keybox}[title={The collapse worry}]
If the residual were only a stronger instance of a blocked route, the
whole classification would be verbal: ``Route~D'' would not deserve its
own lane.  The next clauses show why that collapse does not occur.
\end{keybox}
...
The awareness arc is introduced here not because it is merely
philosophically attractive, but because it is the already-formalized
theorem family whose structure matches the residual conditions forced by
the failure of Routes A--C.

The terms ``awareness,'' ``Alpha,'' ``ground,'' and ``manifestation'' are
not introduced here as free philosophical stipulations: they name formally
developed structures and theorems in the imported awareness arc (the same
arc summarized in \cref{sec:status}).
...
\begin{keybox}[title={The four-stage positive spine}]
\begin{itemize}[leftmargin=1.3em,itemsep=0.45em,parsep=0pt,topsep=2pt]
\item \textbf{Stage~1}---ObserverCorollary / ReflexiveClosure (non-collapse,
      irreducible distance, minimal ternary closure).
\item \textbf{Stage~2}---ReflexiveUnfolding (no terminal completion).
\item \textbf{Stage~3}---AwarenessGround / AlphaNonNull (locus, non-object-level presence).
\item \textbf{Stage~4}---UnifiedPresence / GoldenBridge (ground, articulation, manifestation as one coordinated fact).
\end{itemize}
\end{keybox}
...
We state the \textbf{summit synthesis theorem} whose negative clauses are
discharged by external engines, whose positive clause composes the
awareness arc, and whose route-partition packaging is certified in the thin
\ONE{} layer (\cref{sec:status,app:formalization}).

\begin{theorem}[Observer Non-Exhaustibility]
\label{thm:summit}
\hfill
\begin{enumerate}[label=(\roman*),leftmargin=2.2em]
\item Within the route partition of \Cref{def:routes} (for observer
      architectures in the sense of \Cref{def:obs}), observerhood is not
      internally exhaustible by the principal reductive routes: not by
      parametric self-modeling, not by internal reflective closure, and
      not by total internal certification or final internal self-theory.
...
Clause~(i) combines \Cref{def:routes,def:obs} with \Cref{prop:neg}:
\RI{} blocks Route~A, \RFO{} blocks Route~B, and \ICA{} + \SEM{} block
Route~C.  Clause~(ii) follows
...
analysis on the formal route type, preceded in the same module by the
four-way disjunction \leanref{observer\_exhaustion\_route\_exhaustive\_four}
(Appendix~\ref{tab:one-packaging}).

The thin \Lean{} layer certifies the route vocabulary, bridge wrappers,
summit conjunction, four-way route-tag exhaustiveness, and the
route-partition residual.  The strongest
...
Under the stated route partition and theorem-status discipline
(\cref{sec:status}), observerhood is non-exhaustible along the principal
internal reductive lanes.  What remains is not blank unknowability but an
awareness-grounded residual architecture with a positive and formally
...
architecture is formally identified.  Relative to that reductive route
partition, the observer is shown to be non-exhaustible not by vague appeal
to mystery, but by the joint force of multiple theorem engines and an
awareness-grounded survivor structure.
\end{keybox}
...
supply tools that the present theorem stack now composes.  On the formal
side, G\"odel and Turing fix limits to internal totalization and mechanized
decision on self-referential problems; Lawvere and Yanofsky articulate
diagonal and universality structure; Hofstadter supplies enduring intuition
about strange loops and self-modeling.  On the phenomenological side,
Husserl and Merleau-Ponty redirect self-knowledge toward non-objectifiable
...
\begin{keybox}[title={A crossing point of traditions}]
The synthesis is not ``more of the same'' from any single field.  It is
the point at which obstruction theorems, certification barriers, and
awareness-grounded structure are jointly organized into one
observer-architecture classification.
\end{keybox}
...
\item It does not claim that the current thin \Lean{} packaging layer has
      ...
      wrappers, summit conjunction, four-way exhaustiveness of the route
      tag, and route-partition residual (Appendix~\ref{app:formalization}).  The
...
\begin{keybox}[title={Honest limits as methodological strength}]
Scope is narrow on purpose: consume discharged engines, state what the
\Lean{} packaging certifies, and separate that from omnibus philosophical
claims.  What the paper does not assert is part of what makes what it
does assert checkable and adversarially durable.
\end{keybox}
...
Engines are discharged in external \Lean{} libraries (\cref{sec:status});
\ONE{} contributes the packaging items below.
\begin{enumerate}[label=(\arabic*),leftmargin=2.2em]
...
\item a four-way exhaustiveness lemma and a route-partition residual
      theorem over the formal route vocabulary.
\end{enumerate}

\begin{keybox}[title={Engines vs \ONE{} packaging}]
Heavy proofs live in \RI, \RFO, \ICA/\SEM, and reflexive-closure libraries;
\ONE{} certifies the case partition and import surface (\cref{sec:status}).
\end{keybox}

\subsection*{What \ONE{} proves directly}

Packaging only (Appendix~\ref{app:formalization}): engines stay in \RI,
\RFO, \ICA/\SEM, and awareness libraries.  Prose ``only Route~D'' remains
synthesis-level per \cref{sec:status}.

\medskip
\noindent
The tabular inventory in Appendix~\ref{app:formalization} is the
authoritative crosswalk for packaging declarations as stored in the
\texttt{Observer-Non-Exhaustibility} repository; the consolidated engine
cite index remains \texttt{NOTES/Combined\_Lean\_Manifests.md}.

%% ===================================================================
\section{Conclusion}
\label{sec:conclusion}

Within the principal internal reductive route partition fixed in this
paper, the observer is non-exhaustible (\cref{sec:status,thm:summit}).
...
That is the classification theorem of Observer Non-Exhaustibility,
\emph{under the internal reductive route partition of} \Cref{def:routes}.
The claim is both negative and positive: negative, because it blocks the
...
\section{\texorpdfstring{Formalization inventory (\Lean)}{Formalization inventory (Lean)}}
\label{app:formalization}

\noindent
Table~\ref{tab:one-packaging} lists the \emph{thin packaging} declarations
under namespace \path{Observer.Nonexhaustability} in the
\path{observer-non-exhaustability-lean} submodule. The same rows are
maintained in the outer repository as \path{MANIFEST.md} (\ONE{}
packaging \Lean{} table), \path{ARTIFACT.md}, and\linebreak
\path{NOTES/FORMALIZATION_MAP.md}. Discharged-engine proof obligations
live in the libraries cited in \cref{sec:status}; the concatenated
cross-library manifest is \path{NOTES/Combined_Lean_Manifests.md}.

\medskip
\begin{table}[p]
\centering
\caption{Thin \ONE{} packaging layer: modules and principal declarations.}
\label{tab:one-packaging}
{\footnotesize\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.14\linewidth}
  >{\raggedright\arraybackslash}p{0.52\linewidth}
  >{\raggedright\arraybackslash}p{0.30\linewidth}@{}}
\toprule
\textbf{Layer} & \textbf{Module / declaration} & \textbf{Role} \\
\midrule
Route tag &
  \texttt{Core/Routes.lean}: inductive type \leanref{ObserverExhaustionRoute}. &
  Finite vocabulary for Routes A--D. \\
Route A bridge &
  \texttt{Bridge/FromRI.lean}: \leanref{ParametricSelfModelRouteBlocked};
  \leanref{no\_observer\_exhaustion\_via\_parametric\_self\_model};
  \leanref{ri\_route\_engine}. &
  Imports \RI{} summit surface (no reproof). \\
Route B bridge &
  \texttt{Bridge/FromRFO.lean}:
  \leanref{no\_observer\_exhaustion\_via\_internal\_closure};
  \leanref{rfo\_route\_engine}. &
  Fold / preserved-invariant mismatch packaging. \\
Route C bridge &
  \texttt{Bridge/FromCertification.lean}:
  \leanref{no\_observer\_exhaustion\_via\_total\_internal\_self\_theory};
  \leanref{certification\_route\_engine} (barrier entry point also available). &
  \SEM{} \leanref{no\_final\_self\_theory} lane; ICA/adequacy bridge
  \emph{tracked as repository TODO} when Lake pins allow. \\
Route D exports &
  \texttt{Bridge/FromAwareness.lean}: re-exports including
  \leanref{no\_self\_exhausting\_observer},
  \leanref{closure\_without\_collapse},
  \leanref{irreducible\_reflexive\_distance\_under\_diagonal},
  \leanref{noncollapsing\_reflexive\_closure\_minimally\_ternary},
  \leanref{no\_terminal\_reflexive\_completion},
  \leanref{semantic\_remainder\_implies\_not\_terminal},
  \leanref{unified\_three\_aspects},
  \leanref{golden\_bridge}. &
  Positive residual cite surface (proofs in reflexive-closure suite). \\
Summit conjunction &
  \texttt{Theorems/Classification.lean}:
  \leanref{observer\_non\_exhaustibility\_summit}
  (= \leanref{observer\_non\_exhaustibility\_classification},
  \leanref{only\_awareness\_residual\_route\_remains}). &
  $\wedge$ of the three negative engine \emph{statement shapes} packaged
  for \ONE. \\
Route predicate &
  \texttt{Theorems/Classification.lean}:
  \leanref{AdmissibleAwarenessResidual};
  \leanref{awareness\_residual\_designated\_admissible}. &
  Route~D admissibility predicate (scaffold for future specs). \\
Partition (exhaustive) &
  \texttt{Theorems/Classification.lean}:
  \leanref{observer\_exhaustion\_route\_exhaustive\_four}. &
  Every formal route tag is A$\lor$B$\lor$C$\lor$D (finite menu). \\
Partition (residual) &
  \texttt{Theorems/Classification.lean}:
  \leanref{route\_partition\_residual\_of\_not\_ABC};
  \leanref{observer\_route\_classification\_residual}. &
  Within that menu, $\neg$A$\land\neg$B$\land\neg$C $\Rightarrow$~D. \\
Root import &
  \texttt{Observer/Nonexhaustability.lean} &
  Aggregates \texttt{Core}, \texttt{Bridge.*}, \texttt{Theorems}. \\
\bottomrule
\end{tabular}}
\end{table}

\noindent
\textbf{Scope note.} This appendix records \emph{packaging} only.  It does
not list every lemma in \RI, \RFO, NEMS, or reflexive-closure libraries.

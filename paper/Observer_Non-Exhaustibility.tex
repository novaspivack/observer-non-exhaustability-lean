...
\textbf{(1)~Discharged engines:} base theorems in external \Lean{}
libraries (\RI, \RFO, \SEM, awareness arc).  \textbf{(2)~\ONE{}
packaging:} route type, bridge wrappers, summit conjunction,
four-way exhaustiveness of the route tag, and route-partition residual
in this repository (Appendix~\ref{app:formalization}).  \textbf{(3)~Synthesis
conclusions:} the prose classification that composes those surfaces;
...
\textbf{Stage~1}---ObserverCorollary / ReflexiveClosure (non-collapse,
irreducible distance, minimal ternary closure).  \textbf{Stage~2}---ReflexiveUnfolding
(no terminal completion).  \textbf{Stage~3}---AwarenessGround /
AlphaNonNull (locus, non-object-level presence).  \textbf{Stage~4}---UnifiedPresence
/ GoldenBridge (ground, articulation, manifestation as one coordinated fact).
\end{keybox}
